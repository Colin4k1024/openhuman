...
use crate::store::chunks::{chunk_markdown, ChunkerInput, ChunkerOptions};
...
use crate::tree::score::{self, ScoreResult, ScoringConfig};
            ...
            match std::panic::catch_unwind(move || markdown_body_preview(&md_for_preview)) {
                ...
                Err(_) => {
                    log::error!(
                        ...
                        crate::orchestration::util::redact::redact(source_id)
                    );
                    None
                }
            }
...
mod tests {
    ...
    use crate::store::chunks::types::SourceKind;
    use crate::sync::canonicalize::chat::ChatMessage;
    use crate::tree::score::store::{count_scores, lookup_entity};

...
struct ChatProviderAdapter {
    inner: std::sync::Arc<dyn crate::orchestration::chat::ChatProvider>,
}
...
mod tests {
    ...
    fn make_node(namespace: &str, node_id: &str, summary: &str, child_count: u32) -> TreeNode {
        let level = crate::tree::tree_runtime::types::level_from_node_id(node_id);
        let parent_id =
            crate::tree::tree_runtime::types::derive_parent_id(node_id);
        let ts = Utc::now();
        TreeNode {
            ...
            token_count: crate::tree::tree_runtime::types::estimate_tokens(
                summary,
            ),
            child_count,
            created_at: ts,

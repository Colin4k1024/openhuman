...
pub async fn memory_learn_all(
    ...
    for namespace in &target_ns {
        ...
        let outcome = crate::tree::tree_runtime::ops::tree_summarizer_run(
            &config, namespace,
        )
        .await;
        match outcome {
...
mod tests {
    ...
    async fn seed_namespace(prefix: &str) -> String {
        ...
        let client = crate::orchestration::global::client().expect("memory client");
        client
            .put_doc_light(NamespaceDocumentInput {
                ...
                session_id: None,
                document_id: None,
                taint: crate::orchestration::MemoryTaint::Internal,
            })
            .await

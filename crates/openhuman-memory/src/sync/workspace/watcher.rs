...
use crate::config::{rpc as config_rpc, Config};
...
use crate::bridge::scheduler::gate::current_policy;
...
async fn handle_remove(
    ...
    if let Some(mtime) = last_mtime {
        let source_id = format!("vault_watcher:{rel}@{mtime}");
        if let Err(e) =
            crate::orchestration::ingest_pipeline::mark_document_deleted(config, &source_id)
                .await
        {
            tracing::warn!(
                path = %rel,

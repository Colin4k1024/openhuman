...
use crate::orchestration::ingest_pipeline::ingest_document_with_scope;
...
use crate::sync::composio::{self, ComposioUsage, SyncReason};
        ...
        let inner = tokio::spawn(async move {
            ...
            let outcome = match source.kind {
                ...
                SourceKind::GithubRepo => {
                    ...
                    // audit for this kind.
                    crate::sync::sources::github::run_github_sync(
                        &source, &config,
                    )
                    .await
...
async fn check_and_rebuild_tree(source: &MemorySourceEntry, config: &Config) {
    use crate::sync::sources::rebuild::{needs_rebuild, rebuild_tree_from_raw};

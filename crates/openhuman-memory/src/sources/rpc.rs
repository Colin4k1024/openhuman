...
pub async fn list_rpc() -> Result<RpcOutcome<ListResponse>, String> {
    ...
    crate::sources::reconcile::ensure_composio_sources().await;
...
pub struct SyncAuditLogResponse {
    pub entries: Vec<crate::sync::sources::audit::SyncAuditEntry>,
}

pub async fn sync_audit_log_rpc() -> Result<RpcOutcome<SyncAuditLogResponse>, String> {
    let config = config_rpc::load_config_with_timeout().await?;
    let entries = crate::sync::sources::audit::read_audit_log(&config);
...
pub async fn estimate_sync_cost_rpc(
    ...
    let estimated_cost_usd = crate::sync::sources::audit::estimate_cost_usd(

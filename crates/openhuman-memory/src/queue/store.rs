...
use crate::queue::redact::scrub_for_log;
use crate::queue::types::{Job, JobKind, JobStatus, NewJob};
...
pub fn mark_failed_typed(
    ...
    job: &Job,
    error: &str,
    failure: Option<&crate::queue::health_types::PipelineFailure>,
) -> Result<()> {
    let job_id = &job.id;
...
mod tests {
    ...
    fn mark_failed_typed_unrecoverable_terminates_immediately() {
        use crate::queue::health_types::{FailureCode, PipelineFailure};
        let (_tmp, cfg) = test_config();

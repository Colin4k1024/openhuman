...
use crate::tree::score::embed::{build_write_embedder, pack_checked};
use crate::tree::score::store as score_store;
use crate::tree::tree::store as summary_store;
use crate::tree::tree::{LeafRef, TreeFactory};
            ...
            Some(embedder) => {
                ...
                let vector = match embedder.embed(&body).await {
                    ...
                    Err(e) => {
                        ...
                        let failure =
                            crate::tree::health::classify_embed_error(&e);
        ...
        if let Some(content_path) = chunk_store::get_chunk_content_path(config, &chunk.id)? {
            ...
            if let Err(e) = content_tags::update_chunk_tags(&abs_path, &obsidian_tags) {
                log::warn!(
                    ...
                    chunk.id,
                    crate::orchestration::util::redact::redact(&content_path),
                );
...
async fn handle_append_buffer(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::tree::tree::bucket_seal::should_seal;
...
async fn handle_seal(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::tree::tree::bucket_seal::{seal_one_level, should_seal};
    use crate::tree::tree::store as src_store;
...
async fn handle_flush_stale(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    let buffers = crate::store::trees::store::list_stale_buffers(config, cutoff)?;
    for buf in buffers {
        let seal = SealPayload {
...
async fn handle_reembed_backfill(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    if chunk_ids.is_empty() && summary_ids.is_empty() {
        crate::queue::set_backfill_in_progress(false);
        log::info!(

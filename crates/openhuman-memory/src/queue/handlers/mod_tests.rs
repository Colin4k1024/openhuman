...
async fn reembed_backfill_skips_when_no_provider() {
    ...
    let _health_guard = crate::tree::health::test_guard();
...
async fn ensure_reembed_backfill_enqueues_only_when_uncovered() {
    use crate::queue::ensure_reembed_backfill;
    use crate::store::chunks::store::{upsert_chunks, upsert_staged_chunks_tx};
    use crate::store::chunks::types::{
        chunk_id, Chunk, Metadata, SourceKind, SourceRef,
    };

...
pub fn ensure_reembed_backfill(config: &crate::config::Config) {
    let sig = crate::store::chunks::store::tree_active_signature(config);
    let result = crate::store::chunks::store::with_connection(config, |conn| {
        Ok(crate::store::chunks::store::has_uncovered_reembed_work(conn, &sig)?)
    });
    match result {
        Ok(true) => {

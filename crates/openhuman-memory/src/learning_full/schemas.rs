...
fn handle_rebuild_cache(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let client = crate::orchestration::global::client_if_ready()
...
fn handle_cache_stats(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::learning_full::cache::FacetCache;
        use crate::store::profile::FacetState;
...
fn get_cache() -> Result<crate::learning_full::cache::FacetCache, String> {
    let client = crate::orchestration::global::client_if_ready()
        .ok_or_else(|| "memory client not ready".to_string())?;
    Ok(crate::learning_full::cache::FacetCache::new(
        client.profile_conn(),
    ))
}
...
fn facet_to_json(f: &crate::store::profile::ProfileFacet) -> serde_json::Value {
    serde_json::json!({
        "key": f.key,
        "value": f.value,
...
fn handle_forget_facet(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::store::profile::{FacetState, UserState};

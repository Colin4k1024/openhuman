...
impl StabilityDetector {
    ...
    pub fn rebuild(&self, now: f64) -> anyhow::Result<RebuildOutcome> {
        ...
        for key in existing_by_key.keys() {
            if let Some(class) = crate::learning_full::cache::class_from_key(key) {
        ...
        for ((class, full_key), cands) in &groups {
            ...
            let new_refs: Vec<crate::learning_full::candidate::EvidenceRef> =
...
fn class_prefix(class: FacetClass) -> &'static str {
    crate::learning_full::cache::class_prefix(class)
}
...
mod tests {
    ...
    fn rebuild_pinned_facet_stays_active_regardless_of_stability() {
        ...
        use crate::store::profile::{FacetState, FacetType, UserState};

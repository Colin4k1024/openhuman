...
use crate::bridge::integrations::{build_client, IntegrationClient};
use regex::Regex;
use serde_json::json;
...
fn build_memory_client() -> anyhow::Result<crate::store::MemoryClient> {
    crate::store::MemoryClient::new_local()
...
async fn persist_linkedin_profile(
    memory: &crate::store::MemoryClient,
    url: &str,
    data: &serde_json::Value,
) -> anyhow::Result<()> {
    let content = format!(

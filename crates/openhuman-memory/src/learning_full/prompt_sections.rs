...
use crate::bridge::agent::prompt::{PromptContext, PromptSection};
use anyhow::Result;
...
impl LearnedContextSection {
    pub fn new(_memory: std::sync::Arc<dyn crate::orchestration::Memory>) -> Self {
...
pub fn load_learned_from_cache(
    cache: &crate::learning_full::cache::FacetCache,
) -> Vec<String> {
    ...
    use crate::store::profile::ProfileFacet;
    ...
    'outer: for indices in by_class.values() {
        for &idx in indices {
            ...
            let pinned =
                if f.user_state == crate::store::profile::UserState::Pinned {
                    " *(pinned)*"
                } else {
                    ""
                };
...
mod tests {
    use super::*;
    use crate::bridge::agent::prompt::LearnedContextData;
    ...
    impl Memory for NoopMemory {
        ...
        async fn recall(
            ...
            _query: &str,
            _limit: usize,
            _opts: crate::orchestration::RecallOpts<'_>,
        ...
        async fn namespace_summaries(
            &self,
        ) -> anyhow::Result<Vec<crate::orchestration::NamespaceSummary>> {
            Ok(Vec::new())
        }
    ...
    fn prompt_context(learned: LearnedContextData) -> PromptContext<'static> {
        ...
        PromptContext {
            ...
            learned,
            visible_tool_names,
            tool_call_format: crate::bridge::agent::prompt::ToolCallFormat::PFormat,

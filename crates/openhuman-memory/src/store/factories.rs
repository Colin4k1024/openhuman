...
///
/// Delegates to [`crate::config::ollama_base_url`] so the probe
...
mod tests {
    ...
    impl EnvGuard {
        fn set(value: &str) -> Self {
            let lock = crate::config::inference_test_guard();
    ...
    fn embedding_settings_local_overrides_memory_config() {
        ...
        assert_eq!(
            dims,
            crate::embeddings::DEFAULT_OLLAMA_DIMENSIONS,
    ...
    fn embedding_settings_local_with_empty_model_uses_default() {
        ...
        assert_eq!(
            model,
            crate::embeddings::DEFAULT_OLLAMA_MODEL,
    ...
    fn ollama_health_gate_reports_at_most_once_per_process() {
        let _lock = crate::config::inference_test_guard();
        reset_health_gate_for_test();

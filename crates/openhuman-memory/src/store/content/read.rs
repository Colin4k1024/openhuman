...
fn read_chunk_body_from_raw(
    config: &crate::config::Config,
    refs: &[crate::store::chunks::store::RawRef],
) -> anyhow::Result<String> {
    let content_root = config.memory_tree_content_root();
...
mod tests {
    ...
    use crate::store::chunks::store::{upsert_chunks, with_connection};
    use crate::store::chunks::types::{Chunk, Metadata, SourceKind};
    use crate::store::content::atomic::{sha256_hex, write_if_new};
    use crate::store::content::compose::{
        compose_chunk_file, SummaryComposeInput,
    };
    use crate::store::content::paths::SummaryTreeKind;
    use crate::store::content::{atomic::stage_summary, stage_chunks};
    ...
    fn read_chunk_body_errors_on_sha_mismatch() {
        ...
        let rel =
            crate::store::chunks::store::get_chunk_content_path(&cfg, &chunk.id)
                .unwrap()
                .unwrap();

...
use super::*;
use crate::store::chunks::types::chunk_id;
use chrono::TimeZone;
use rusqlite::params;
...
fn clear_memory_delete_keeps_tree_when_another_connection_still_owns_chunks() {
    use crate::store::trees::store as tree_store;
    use crate::store::trees::types::{Buffer, Tree, TreeKind, TreeStatus};
...
fn legacy_embeddings_migrate_to_sidecar_once() {
    ...
    let (p, m, dims) = crate::store::effective_embedding_settings(
        &cfg.memory,
        cfg.workload_local_model("embeddings").as_deref(),
    );
    let sig = crate::embeddings::format_embedding_signature(&p, &m, dims);
...
fn clear_reembed_skipped_for_signature_removes_all_tombstones_for_sig() {
    ...
    .unwrap();
    crate::store::trees::store::mark_summary_reembed_skipped(
        &cfg,
        summary_id,
        &sig,
        "summary tombstone",

...
//! [`crate::tree::score::embed::pack_embedding`]. New
...
use crate::tree::score::embed::{decode_optional_blob, pack_checked};
...
///
/// Cascade order mirrors [`crate::store::chunks::store`]'s
...
/// [`crate::store::chunks::store::tree_active_signature`] — same
...
pub fn set_summary_embedding(
    ...
    let signature = crate::store::chunks::store::tree_active_signature(config);
    log::debug!(
...
pub fn mark_summary_reembed_skipped(
    ...
    let summary_id = crate::store::chunks::store::validate_reembed_skip_key(
        "summary_id",
        summary_id,
    )?;
    let model_signature = crate::store::chunks::store::validate_reembed_skip_key(
        "model_signature",
        model_signature,
    )?;
    with_connection(config, |conn| {
...
///
/// Idempotent — see [`crate::store::chunks::store::clear_chunk_reembed_skipped`].

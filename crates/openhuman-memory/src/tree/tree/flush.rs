...
pub async fn flush_stale_buffers(
    ...
    use crate::store::trees::store::get_trees_batch;
...
mod tests {
    ...
    // TODO: re-enable when orchestration layer is moved
    ...
    use crate::tree::tree::bucket_seal::{append_leaf, LeafRef};
    use std::sync::Arc;
    ...
    async fn flush_does_not_force_seal_under_fanout_upper_buffer() {
        ...
        crate::store::chunks::store::with_connection(&cfg, |conn| {
            let tx = conn.unchecked_transaction()?;
            crate::store::trees::store::upsert_buffer_tx(
                &tx,
                &crate::store::trees::types::Buffer {

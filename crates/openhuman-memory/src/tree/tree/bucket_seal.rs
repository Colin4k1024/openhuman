...
use crate::tree::score::resolver::canonicalise;
use crate::tree::summarise::{
    fallback_summary, summarise, SummaryContext, SummaryInput,
};
use crate::tree::tree::factory::TreeFactory;
use crate::tree::tree::registry::new_summary_id;
...
/// by `flush_stale_buffers` (see [`crate::tree::
...
pub(crate) async fn seal_one_level(
        ...
        Some(embedder) => {
            let v = match embedder.embed(&embed_input).await {
                ...
                Err(e) => {
                    ...
                    let failure = crate::tree::health::classify_embed_error(&e);
            ...
            crate::tree::score::embed::pack_checked(&v).context(format!(
    ...
    let child_basename_overrides: Option<Vec<Option<String>>> = if node.level == 1 {
        let overrides: Vec<Option<String>> = node
            ...
            .map(|chunk_id| {
                ...
                match crate::store::chunks::store::get_chunk_raw_refs(
                    config, chunk_id,
                ) {
    ...
    with_connection(config, move |conn| {
        ...
        store::insert_summary_tx(
            ...
            &node,
            Some(&staged),
            &crate::store::chunks::store::tree_active_signature(config),
        )?;
        ...
        crate::tree::score::store::index_summary_entity_ids_tx(
            &tx,
            &node.entities,
            &node.id,
            node.score,
        ...
        if enqueue_follow_ups {
            ...
            if should_seal(&parent) {
                use crate::queue::store::enqueue_tx as enqueue_job_tx;
                use crate::queue::types::{NewJob, SealPayload};
...
fn truncate_for_embed(text: &str, max_tokens: u32) -> String {
    let approx = crate::store::chunks::types::approx_token_count(text);
...
fn hydrate_leaf_inputs(config: &Config, chunk_ids: &[String]) -> Result<Vec<SummaryInput>> {
    use crate::store::chunks::store::get_chunk;
    use crate::store::content::read as content_read;
    use crate::tree::score::store::{get_score, list_entity_ids_for_node};
...
fn hydrate_summary_inputs(config: &Config, summary_ids: &[String]) -> Result<Vec<SummaryInput>> {
    use crate::store::content::read as content_read;
    use crate::store::trees::store::get_summaries_batch;

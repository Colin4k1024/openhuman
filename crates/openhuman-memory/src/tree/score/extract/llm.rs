...
use crate::bridge::inference::{ChatPrompt, ChatProvider};
...
/// [`crate::bridge::inference::build_chat_provider`]). Tests
...
impl EntityExtractor for LlmEntityExtractor {
    ...
    async fn extract(&self, text: &str) -> anyhow::Result<ExtractedEntities> {
            ...
            match self.try_extract(text).await {
                Some(extracted) => {
                    ...
                    if !extracted.entities.is_empty() || !extracted.topics.is_empty() {
                        // health: clear_structure_degraded (gated)
                    }
                    return Ok(extracted);
                }
                None => {
        ...
        log::warn!("structure_degraded: ExtractionTimeout");
...
fn build_system_prompt(emit_topics: bool, output_language: Option<&str>) -> String {
    ...
    let language_directive = crate::config::output_language_directive(output_language)

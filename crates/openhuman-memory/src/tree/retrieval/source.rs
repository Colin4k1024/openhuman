...
use crate::store::trees::types::{SummaryNode, Tree, TreeKind};
use crate::tree::retrieval::types::{
    hit_from_summary, QueryResponse, RetrievalHit,
};
use crate::tree::score::embed::{build_embedder_from_config, cosine_similarity};
...
mod tests {
    ...
    async fn seed_source(cfg: &Config, scope: &str, ts: DateTime<Utc>) {
        ...
        for seq in 0..2u32 {
            let c = Chunk {
                ...
                token_count: crate::store::trees::types::INPUT_TOKEN_BUDGET * 6
                    / 10,
                seq_in_source: seq,
                created_at: ts,
            ...
            crate::store::chunks::store::with_connection(cfg, |conn| {
                ...
                crate::store::chunks::store::upsert_staged_chunks_tx(
                    &tx, &staged,
                )?;
                tx.commit()?;
                Ok(())
    ...
    async fn query_reranks_by_cosine_similarity() {
        use crate::tree::score::embed::{pack_embedding, EMBEDDING_DIM};
        ...
        let phoenix_tree = src_store::get_tree_by_scope(
            &cfg,
            crate::store::trees::types::TreeKind::Source,
            "slack:#phoenix",
        )
        .unwrap()
        .unwrap();
        ...
        use crate::tree::score::embed::cosine_similarity;

...
use crate::bridge::inference::ChatProvider;
use crate::tree::tree_runtime::store;
use crate::tree::tree_runtime::types::{
...
async fn summarize_to_limit(
    ...
    let response = if response.len() > char_limit {
        ...
        let truncated =
            &response[..crate::tree::util::floor_char_boundary(&response, char_limit)];
        truncated.to_string()
    } else {
        response
    };
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        } else if ft.is_file() && name.ends_with(".md") && name != "summary.md" && name != "root.md"
        {
            ...
            if level == NodeLevel::Hour {
                ...
                let node =
                    crate::tree::tree_runtime::store::parse_node_markdown_pub(
...
pub async fn run_hourly_loop(config: Config, provider: Box<dyn ChatProvider>, model: String) {

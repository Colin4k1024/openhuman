...
use crate::tree::tree_runtime::{engine, store, types::*};
...
fn create_provider(
    ...
    (
        Box<dyn crate::bridge::inference::ChatProvider>,
        String,
    ),
    String,
> {
...
///
/// In standalone mode, this returns an error — the full app provides the real
/// implementation via the inference bridge.
fn create_local_ai_provider(
    _config: &Config,
) -> Result<Box<dyn crate::bridge::inference::ChatProvider>, String> {
    Err("local AI provider not available in standalone mode".to_string())
}
